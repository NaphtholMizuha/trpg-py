...
from trpg_py.agent.tools import create_lint_tool, lint_task_document
...
class LintToolTests(unittest.TestCase):
    ...
    def test_lint_tool_reports_tool_errors_separately(self) -> None:
        ...
        with patch("trpg_py.agent.tools.lint.lint_task_document", side_effect=RuntimeError("lint backend failed")):
            payload = tool.invoke(
                {
                    "task_document": {
                        "task_id": "demo",
