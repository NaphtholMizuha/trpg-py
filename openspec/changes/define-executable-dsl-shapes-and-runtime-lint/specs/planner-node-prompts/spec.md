...
#### 场景:dsl_node prompt 显式声明所有合法 DSL 形状
- **当** `dsl_node` 构造默认 prompt
- **那么** prompt 必须能显式覆盖当前受支持 primitive 的 canonical 合法形状
- **那么** prompt 必须能显式指出常见禁止形状，而不是只给 primitive 名称列表
- **那么** prompt 中的形状描述必须与系统维护的合法 DSL 形状目录保持一致
