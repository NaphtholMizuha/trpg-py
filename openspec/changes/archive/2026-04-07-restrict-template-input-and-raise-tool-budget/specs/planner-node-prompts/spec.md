...
- **那么** `dsl_node` 默认 prompt 必须要求模型使用文档化的精确 template 枚举值，而不是近义自由拼写
- **那么** `dsl_node` 默认 prompt 必须把 `unknown` 说明为唯一合法的保守回退值
...
- **那么** prompt 必须明确列出合法 template 枚举词表或等价约束
- **那么** prompt 必须要求模型使用精确文档值，不得输出未文档化近义值如 `area_spell`
...
- **那么** prompt 不得把未文档化的近义值描述成可接受的回退手段
