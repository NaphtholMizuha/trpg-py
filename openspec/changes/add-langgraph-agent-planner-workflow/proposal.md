...
- 收紧第一阶段节点的工具边界，使 `task_node` 默认只拥有 `grep` 与可选 `search` 这类检索能力，而不再直接持有 `read` 权限。
...
- 受影响代码：`src/augury/planner/workflow.py`、`src/augury/planner/nodes/`、planner 共享类型以及与 tools 的接线方式，尤其是 `task_node` 的默认工具权限。
